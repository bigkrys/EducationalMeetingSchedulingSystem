...
async function getAppointmentsHandler(request: NextRequest, context?: any) {
  try {
    ...
    // 查询预约
    const appointments = await span('db appointment.findMany', () =>
...
async function createAppointmentHandler(request: NextRequest, context?: any) {
  try {
    ...
    // 检查幂等性
    const existingAppointment = await span('db appointment.findUnique', () =>
      prisma.appointment.findUnique({ where: { idempotencyKey: validatedData.idempotencyKey } })
    )

    if (existingAppointment) {
      // 已存在相同 idempotencyKey 的预约：返回现有预约信息以支持幂等重试
      const apt = await span('db appointment.findUnique', () =>
    ...
    // 检查学生是否存在且为激活状态
    const student = await span('db student.findUnique', () =>
      prisma.student.findUnique({ where: { id: validatedData.studentId }, include: { user: true } })
    )
    ...
    // 检查教师是否存在
    const teacher = await span('db teacher.findUnique', () =>
      prisma.teacher.findUnique({ where: { id: validatedData.teacherId }, include: { user: true } })
    )
    ...
    // 检查科目是否匹配
    const teacherSubject = await span('db teacherSubject.findFirst', () =>
    ...
    // 检查学生是否已注册该科目
    const studentSubject = await span('db studentSubject.findFirst', () =>
    ...
    let subject = await span('db subject.findFirst', () =>
      prisma.subject.findFirst({ where: { name: validatedData.subject } })
    )
    if (!subject) {
      subject = await span('db subject.create', () =>
    ...
    try {
      appointment = await span('db appointment.create.tx', () =>
    ...
    // 确保教师有该科目
    const existingTeacherSubject = await span('db teacherSubject.findUnique', () =>
      prisma.teacherSubject.findUnique({
        where: {
          teacherId_subjectId: { teacherId: validatedData.teacherId, subjectId: subject.id },
        },
      })
    )
    if (!existingTeacherSubject) {
      await span('db teacherSubject.create', () =>
    ...
    await span('cache deletePattern', () =>
      deleteCachePattern(`slots:${validatedData.teacherId}:${dateStr}:*`)
    )

    // 发送邮件通知
    try {
      // 获取科目名称
      const subjectRecord = await span('db subject.findUnique', () =>
        prisma.subject.findUnique({ where: { id: subject.id } })
      )
...
export const GET = withRoles(['student', 'teacher'])(
  withSentryRoute(getAppointmentsHandler, 'api GET /api/appointments')
)
export const POST = withRateLimit({ windowMs: 60 * 1000, max: 30 })(
  withRole('student')(withSentryRoute(createAppointmentHandler, 'api POST /api/appointments'))
)
