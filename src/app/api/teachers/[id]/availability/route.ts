...
async function detectAvailabilityConflicts(
  ...
  const DEBUG_AV =
    ...
    (env.DEBUG_AVAILABILITY_LOGS || 'false').toLowerCase() === 'true'
  ...
  if (!timeValidation.isValid) {
    if (DEBUG_AV)
      logger.debug('availability.detect invalid_time', { message: timeValidation.message })
    conflicts.push({
      type: 'invalid_time',
  ...
  if (DEBUG_AV) {
    ...
    existingSlots.forEach((slot, index) => {
      logger.debug('availability.detect slot', {
        index: index + 1,
  ...
  for (const slot of existingSlots) {
    if (DEBUG_AV)
      logger.debug('availability.detect check_slot', { start: slot.startTime, end: slot.endTime })
    const overlapType = analyzeTimeOverlap(
      slot.startTime,
      slot.endTime,
      request.startTime,
      request.endTime
...
function analyzeTimeOverlap(
  ...
  const DEBUG_AV =
    ...
    (env.DEBUG_AVAILABILITY_LOGS || 'false').toLowerCase() === 'true'
  if (DEBUG_AV)
    logger.debug('availability.overlap.analyze', { start1, end1, start2, end2, s1, e1, s2, e2 })
  ...
  if (DEBUG_AV)
    logger.debug('availability.overlap.partial', { overlapStart, overlapEnd, overlapMinutes })
...
function generateTimeOptimizationSuggestions(
  ...
  // 只处理周循环的时间段
  const weeklySlots = existingSlots.filter(
    (slot: TeacherAvailability) => slot.dayOfWeek !== undefined
  )
  ...
  try {
    ...
    return ok({
      ...
      summary: {
        ...
        totalHours: availability
          .reduce((sum: number, slot: TeacherAvailability) => {
...
async function setAvailabilityHandler(request: AuthenticatedRequest, context?: any) {
  try {
    ...
    // 检查是否与阻塞时间冲突
    const blockedTimeConflicts = await checkBlockedTimeConflicts(targetTeacher.id, validatedData)
    ...
    // 检查是否与已有预约冲突
    const appointmentConflicts = await checkAppointmentConflicts(targetTeacher.id, validatedData)
    ...
    if (validatedData.dayOfWeek === undefined) {
      return fail(
        'dayOfWeek is required for weekly recurring availability',
        400,
        E.AVAILABILITY_INVALID_MODE
      )
    }
  ...
  try {
    ...
    for (const slot of body.slots) {
      try {
        ...
        // 检查冲突
        const { conflicts } = await detectAvailabilityConflicts(teacherId, validatedData)
    ...
    logger.error('availability.batch.exception', {
