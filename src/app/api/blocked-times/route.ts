...
async function getBlockedTimesHandler(request: AuthenticatedRequest, context?: any) {
  try {
    ...
    logger.error('blocked_times.list.exception', {
...
async function createBlockedTimeHandler(request: AuthenticatedRequest, context?: any) {
  try {
    ...
    if (conflictingAvailability.length > 0) {
      ...
      logger.info('blocked_times.create.availability_conflicts', {
        count: availabilityConflicts.length,
      })
    }
    ...
    if (conflictingAppointments.length > 0) {
      ...
      return fail(
        `阻塞时间与现有预约冲突：${conflictDetails}`,
        409,
        E.BLOCKEDTIME_CONFLICT_APPOINTMENT,
    ...
    logger.error('blocked_times.create.exception', {
      ...getRequestMeta(request),
      error: String(error),
    })
...
async function deleteBlockedTimeHandler(request: AuthenticatedRequest, context?: any) {
  try {
    ...
    logger.error('blocked_times.delete.exception', {
      ...getRequestMeta(request),
      error: String(error),
    })
